...
const PageIntro = ({CName}) => {
  ...
  useEffect(() => {
    ...
    gsap.to(counter, {
      ...
      onUpdate: () => {
        if (numRef.current) {
          numRef.current.textContent = Math.floor(counter.value);
        }
      },
    });
  ...
  // MOUSE MOVE MID ANIMATION
  useEffect(() => {
    const mid = midRef.current;

    let target = { x: 0, y: 0 };
    let current = { x: 0, y: 0 };
    const maxMove = 20; // move limit
    ...
    const handleMove = (e) => {
      ...
      // mouse position difference from center (-1 to 1)
      const dx = (e.clientX - w) / w;
      const dy = (e.clientY - h) / h;

      target.x = dx * maxMove * -1; // invert so right → left move
      target.y = dy * maxMove * -1; // invert so down → up move
    };
  ...
  return (
      ...
      >
        ...
        >
          <path
            ...
            clipRule="evenodd"
            //   fill="#ffffff"
            d={`M510.615,191.984c-0.578,3.031-1.329,6.031-2.229,8.984
	...
	C354.992,375.164,363.577,367.992,372.514,367.992L372.514,367.992z`}
        ...
        <div className="w-fit mid h-4.7 text-white flex justify-center items-center mt-4 overflow-hidden">
          <p className="text-[1.2rem] Text_animater_1 flex translate-y-20 Font2">
            An architecture approach to design
          </p>
        </div>
        <div className="w-fit mid h-4 text-white flex justify-center items-center mt-2 overflow-hidden">
          <p className="text-[1rem] Text_animater_1 flex translate-y-20 Font1">
            Los Angeles CA
          </p>
        </div>
      </div>
      ...
      {/* Bottom */}
      <div className=" absolute btm bottom-0 left-0 w-full h-fit px-8 py-8 flex justify-between items-end">
        {/*  */}
        <div className="w-fit h-8 text-white BT flex justify-center opacity-0  items-center overflow-hidden">
          <p className="text-[1.5rem] Text_animater_2 flex  Font2">(Loading)</p>
        </div>
        <div className="w-fit h-4 text-white BT flex justify-center  opacity-0 items-center overflow-hidden">
          <p className="text-[1.1rem] Text_animater_2 flex  Font1">lusano</p>
        </div>
        <div className="w-fit h-8 text-white BT flex justify-center opacity-0  items-center overflow-hidden">
          <p className="text-[1.5rem] Text_animater_2 flex  Font2">
